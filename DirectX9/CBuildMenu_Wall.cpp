...
CBuildMenu_Wall::CBuildMenu_Wall()
{
	for (int i = 0; i < MENU_WALL_NUM; i++)
	{
		...
		m_pVtxBuffer_Build_Wall[i] = NULL;	//頂点パッファへのポインタ
	}
	m_bUse = false;
...
HRESULT CBuildMenu_Wall::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float height, float width, LPCSTR TEXTURE_FILE)
{
	...
	D3DXCreateTextureFromFile(device,				// デバイスへのポインタ
		"data\\TEXTURE\\wall_001.jpg",		// ファイルの名前
		&m_pTexture_Build_Wall[0]);	// 読み込むメモリー

								// テクスチャの読み込み
	D3DXCreateTextureFromFile(device,				// デバイスへのポインタ
		"data\\TEXTURE\\wall_002.jpg",		// ファイルの名前
		&m_pTexture_Build_Wall[1]);	// 読み込むメモリー
	...
	for (int i = 0; i < MENU_WALL_NUM; i++)
	{
		...
		if (FAILED(device->CreateVertexBuffer(sizeof(VERTEX_2D) * 4 * 1,	// 頂点データ用に確保するバッファサイズ(バイト単位)
			...
			&m_pVtxBuffer_Build_Wall[i],						// 頂点バッファインターフェースへのポインタ
			NULL)))									// NULLに設定
		{
			return E_FAIL;
		}
