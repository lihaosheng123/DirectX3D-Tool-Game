...
//  戻り値　：HRESULT
//=============================================================================
HRESULT CGameEnemy_002::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float height, float width, LPCSTR TEXTURE_FILE)
{
...
//  戻り値　：
//=============================================================================
void CGameEnemy_002::Uninit(void)
{
	CManager *manager = GetManager();
...
//------------------------------------------------------------------------------
//	関数名:	void CGameEnemy_002::Draw
//	引数:	なし
//	戻り値:	なし
//	説明:	描画処理
//------------------------------------------------------------------------------
void CGameEnemy_002::Draw(void)
{
	CManager *manager = GetManager();
	CGame *game = (CGame*)CManager::GetMode();
	CRenderer *renderer = manager->GetRenderer();
...
//------------------------------------------------------------------------------
//	関数名:	void CGameEnemy_002::Update
//	引数:	なし
//	戻り値:	なし
//	説明:	描画処理
//------------------------------------------------------------------------------
void CGameEnemy_002::Update(void)
{
	CManager *manager = GetManager();
	CGame *game = (CGame*)CManager::GetMode();
	...
	if (manager->m_Build == false)
	{
		for (int i = 0; i < m_LoadMap->m_Enemy001_num; i++)
		{
			...
			{
				//→
				if (m_LoadMap->m_Pos_Enemy_002[i].x < m_LoadMap->m_Pos_To_Enemy_002[i].x)
				{
					if (m_data[i].m_Position.x <= m_LoadMap->m_Pos_Enemy_002[i].x)
					{
						m_data[i].m_bMove_Muki = true;
					}

					else if (m_data[i].m_Position.x >= m_LoadMap->m_Pos_To_Enemy_002[i].x)
					{
				...
				//↑
				if (m_LoadMap->m_Pos_Enemy_002[i].z < m_LoadMap->m_Pos_To_Enemy_002[i].z)
				{
					if (m_data[i].m_Position.z <= m_LoadMap->m_Pos_Enemy_002[i].z)
					{
						m_data[i].m_bMove_Muki = true;
					}

					else if (m_data[i].m_Position.z >= m_LoadMap->m_Pos_To_Enemy_002[i].z)
					{
						m_data[i].m_bMove_Muki = false;
					}
...
CGameEnemy_002* CGameEnemy_002::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float height, float width, LPCSTR TEXTURE_FILE)
{
	CGameEnemy_002 *GameEnemy_002;
	GameEnemy_002 = new CGameEnemy_002;
	GameEnemy_002->Init( pos,  rot,  height,  width,  TEXTURE_FILE);
	return GameEnemy_002;
}
void CGameEnemy_002::SetGameEnemy_002(D3DXVECTOR3 pos, D3DXVECTOR3 rot)
{
	CManager *manager = GetManager();
	CGame *game = (CGame*)CManager::GetMode();
	CRenderer *renderer = manager->GetRenderer();
...
CGameEnemy_002_data* CGameEnemy_002::Get(int Index)
{
	return &m_data[Index];
}
float  CGameEnemy_002::Length(D3DXVECTOR3 *Vector)
{
