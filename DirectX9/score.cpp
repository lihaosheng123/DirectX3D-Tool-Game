...
//  戻り値　：HRESULT
//=============================================================================
HRESULT CScore ::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot,float height ,float width,LPCSTR TEXTURE_FILE)
{
	...
	D3DXCreateTextureFromFile(device,				// デバイスへのポインタ
	 ...
	 //オブジェクトの頂点バッファを生成
    if(FAILED(device->CreateVertexBuffer(sizeof(VERTEX_2D) * 4 * MAX_SCORE,	// 頂点データ用に確保するバッファサイズ(バイト単位)
	...
	{//頂点バッファの中身を埋める
		...
		for(int nCntScene = 0; nCntScene < MAX_SCORE; nCntScene++, pVtx += 4)
		{
			// 頂点座標の設定
...
CScore* CScore::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot,float height ,float width,LPCSTR TEXTURE_FILE)
{
	CScore *score;
	score = new CScore;
