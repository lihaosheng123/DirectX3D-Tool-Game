...
//  戻り値　：HRESULT
//=============================================================================
HRESULT CScene2D ::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot,float height ,float width,LPCSTR TEXTURE_FILE)
{
	...
	D3DXCreateTextureFromFile(device,				// デバイスへのポインタ
	 ...
	 //オブジェクトの頂点バッファを生成
    if(FAILED(device->CreateVertexBuffer(sizeof(VERTEX_2D) * 4 * NUM_PLACE_SCENE_2D,	// 頂点データ用に確保するバッファサイズ(バイト単位)
	...
	{//頂点バッファの中身を埋める
		...
		for(int nCntScene = 0; nCntScene < NUM_PLACE_SCENE_2D; nCntScene++, pVtx += 4)
		{
			// 頂点座標の設定
...
void CScene2D :: Update(void)
{
	...
	if(m_Input->GetKeyboardTrigger(DIK_DELETE))
	{
		Uninit();
		Release();
	}
...
CScene2D* CScene2D::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot,float height ,float width,LPCSTR TEXTURE_FILE)
{
	CScene2D *scene2D;
	scene2D = new CScene2D;
	scene2D->Init(pos,rot,height ,width,TEXTURE_FILE);
	return scene2D;
}
