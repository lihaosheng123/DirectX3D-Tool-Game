...
//  戻り値　：HRESULT
//=============================================================================
HRESULT CTimeScore ::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot,float height ,float width,LPCSTR TEXTURE_FILE)
{
	...
	D3DXCreateTextureFromFile(device,				// デバイスへのポインタ
	 ...
	 //オブジェクトの頂点バッファを生成
    if(FAILED(device->CreateVertexBuffer(sizeof(VERTEX_2D) * 4 * MAX_TIMESCORE,	// 頂点データ用に確保するバッファサイズ(バイト単位)
	...
	{//頂点バッファの中身を埋める
		...
		for(int nCntScene = 0; nCntScene < MAX_TIMESCORE; nCntScene++, pVtx += 4)
		{
			// 頂点座標の設定
...
//  戻り値　：
//=============================================================================
void CTimeScore :: Update(void)
{
	CManager *manager = GetManager();
	CGame *game = (CGame*)CManager:: GetMode();
	CNumber *m_Num = game->GetNum();
	CSceneInput *m_Input = manager -> GetInput();
	CTime *m_Time = game->GetTime();
	if(m_Time->m_frame>=60)
	{
		m_Time->m_time ++;
		m_Time->m_frame = 0;
		//AddScore(-1);
	...
	nTimebg[0] =m_nScore / 10 % 10;		//10
	nTimebg[1] =m_nScore / 1 % 10;		//1
	//頂点バッファの中身を埋める
		VERTEX_2D *pVtx;
		// 頂点データの範囲をロックし、頂点バッファへのポインタを取得
		m_pVtxBuffer->Lock(0, 0, (void**)&pVtx, 0);
		...
		{
			// テクスチャ座標の設定
			pVtx[0].tex = D3DXVECTOR2(0.1f * nTimebg[ nCntScene ], 0.0f );
			pVtx[1].tex = D3DXVECTOR2(0.1f * nTimebg[ nCntScene ] + 0.1f, 0.0f );
			pVtx[2].tex = D3DXVECTOR2(0.1f * nTimebg[ nCntScene ], 1.0f );
			pVtx[3].tex = D3DXVECTOR2(0.1f * nTimebg[ nCntScene ] + 0.1f, 1.0f );
		}
...
CTimeScore* CTimeScore::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot,float height ,float width,LPCSTR TEXTURE_FILE)
{
	CTimeScore *score;
	score = new CTimeScore;
	score->Init(pos,rot,height ,width,TEXTURE_FILE);
	return score;
}
void CTimeScore::AddScore(int value)
{
	m_nScore += value;
	if(value <=0)
	{
