...
HRESULT CExplosion::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float height, float width, LPCSTR TEXTURE_FILE)
{
	...
	for (int i = 0; i<EXPLOSION_MAX; i++)
	{
		...
		m_Explosion[i].m_Position = pos;	// 頂点座標
		m_Explosion[i].m_Rotation = rot;	// 回転
		...
		m_Explosion[i].nCount = 0;			// カウンタ
		m_Explosion[i].nPattern = 0;		// パターンNo.	
	...
	{
		pVtx[0].pos = D3DXVECTOR3(-m_Explosion[i].m_width / 2, m_Explosion[i].m_height, 0.0f);	// 頂点座標
...
void CExplosion::Draw(void)
{
	...
	for (int i = 0; i<EXPLOSION_MAX; i++)
	{
		if (m_Explosion[i].m_Buse == true)
		{
			...
			m_Explosion[i].m_mtxWorld._41 = 0.0f;		// 移動に関する変数をクリア
			m_Explosion[i].m_mtxWorld._42 = 0.0f;		// 移動に関する変数をクリア
			m_Explosion[i].m_mtxWorld._43 = 0.0f;		// 移動に関する変数をクリア
													// スケールを反映
			...
			// 回転を反映
			D3DXMatrixRotationYawPitchRoll(&m_Explosion[i].mtxRot, m_Explosion[i].m_Rotation.y, m_Explosion[i].m_Rotation.x, m_Explosion[i].m_Rotation.z);	// ビルボードは常に正面をカメラに向ける2Dポリゴンだから、オール0.0の初期値でいい。
