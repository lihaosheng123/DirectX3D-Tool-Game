...
CGameWall_001* CGameWall_001::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float height, float width, LPCSTR TEXTURE_FILE)
{
	CGameWall_001 *m_GameWall_001;
	m_GameWall_001 = new CGameWall_001;
	m_GameWall_001->Init(pos, rot, height, width, TEXTURE_FILE);
...
//  戻り値　：HRESULT
//=============================================================================
HRESULT CGameWall_001::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float height, float width, LPCSTR TEXTURE_FILE)
{
	...
	if (manager->m_Build == false)
	{
		...
		if (FAILED(device->CreateVertexBuffer(
			sizeof(VERTEX_3D) * m_LoadMap->m_Wall001_num * 4,
			D3DUSAGE_WRITEONLY,
			FVF_VERTEX_3D,
			D3DPOOL_MANAGED,
			&m_pVtxBuffer,
		...
		for (int i = 0; i < m_LoadMap->m_Wall001_num; i++, pVtx += 4)
		{
	...
	if (manager->m_Build == true)
	{
		//頂点バッファの生成
		if (FAILED(device->CreateVertexBuffer(
			sizeof(VERTEX_3D) * m_BuildLoadMap->m_Wall001_num * 4,
			D3DUSAGE_WRITEONLY,
			FVF_VERTEX_3D,
			D3DPOOL_MANAGED,
			&m_pVtxBuffer,
		...
		m_pVtxBuffer->Lock(0, 0, (void**)&pVtx, 0);
		for (int i = 0; i < m_BuildLoadMap->m_Wall001_num; i++, pVtx += 4)
		{
			pVtx[0].pos = D3DXVECTOR3(-m_width / 2, m_height, 0.0f);	// 頂点座標
			pVtx[1].pos = D3DXVECTOR3(m_width / 2, m_height, 0.0f);
			pVtx[2].pos = D3DXVECTOR3(-m_width / 2, 0.0f, 0.0f);
...
//  戻り値　：
//=============================================================================
void CGameWall_001::Update(void)
{
	CManager *manager = GetManager();
...
//=============================================================================
//  関数名　：Uninit
//  機能概要：CGameWall_001の終了処理
//  戻り値　：
//=============================================================================
void CGameWall_001::Uninit(void)
{
	CManager *manager = GetManager();
...
void CGameWall_001::SetGameWall_001(D3DXVECTOR3 pos, D3DXVECTOR3 rot)
{
	CManager *manager = GetManager();
