...
CTitleMenu::CTitleMenu()
{
	for (int i = 0; i < 3; i++)
	{
		m_pTexture_Title[i] = NULL;
		m_pVtxBuffer_Title[i] = NULL;	//頂点パッファへのポインタ
	}
	m_bUse = true;
}
//=============================================================================
...
//  戻り値　：HRESULT
//=============================================================================
HRESULT CTitleMenu::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float height, float width, LPCSTR TEXTURE_FILE)
{
	...
	D3DXCreateTextureFromFile(device,				// デバイスへのポインタ
		"data\\TEXTURE\\title_start.png",		// ファイルの名前
		&m_pTexture_Title[0]);	// 読み込むメモリー


								// テクスチャの読み込み
	D3DXCreateTextureFromFile(device,				// デバイスへのポインタ
		"data\\TEXTURE\\title_build.png",		// ファイルの名前
		&m_pTexture_Title[1]);	// 読み込むメモリー


								// テクスチャの読み込み
	D3DXCreateTextureFromFile(device,				// デバイスへのポインタ
		"data\\TEXTURE\\title_end.png",		// ファイルの名前
		&m_pTexture_Title[2]);	// 読み込むメモリー
	for (int i = 0; i < 3; i++)
	{
		//オブジェクトの頂点バッファを生成
		if (FAILED(device->CreateVertexBuffer(sizeof(VERTEX_2D) * 4 * 1,	// 頂点データ用に確保するバッファサイズ(バイト単位)
			...
			&m_pVtxBuffer_Title[i],						// 頂点バッファインターフェースへのポインタ
			NULL)))									// NULLに設定
		{
			return E_FAIL;
		}
		...
		{//頂点バッファの中身を埋める
			...
			m_pVtxBuffer_Title[i]->Lock(0, 0, (void**)&pVtx, 0);
...
void CTitleMenu::Uninit(void)
{
	for (int i = 0; i < 3; i++)
	{
		RELEASE(m_pTexture_Title[i]);
		RELEASE(m_pVtxBuffer_Title[i]);
	}
}
//=============================================================================
//  関数名　：Draw
...
void CTitleMenu::Draw(void)
{
	...
	if (m_bUse == true)
	{
		for (int i = 0; i < 3; i++)
		{
			...
			//テクスチャの設定
			device->SetTexture(0, m_pTexture_Title[i]);

			// 頂点バッファをデータストリームにバインドする
			device->SetStreamSource(
				0,
				m_pVtxBuffer_Title[i],
				0,
				sizeof(VERTEX_2D)
			);
			// フェードの描画設定内容
...
CTitleMenu* CTitleMenu::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float height, float width, LPCSTR TEXTURE_FILE)
{
	CTitleMenu *m_titleMenu;
	m_titleMenu = new CTitleMenu;
	m_titleMenu->Init(pos, rot, height, width, TEXTURE_FILE);
	return m_titleMenu;
}
